...
import { CheckCircle, Package, MapPin, Clock, Phone, Mail, MessageCircle, User, Truck, Store } from "lucide-react"
...
export default function CheckoutSuccessPage() {
    ...
    const fetchAndUpdateOrder = async () => {
      ...
      try {
        ...
        if (updateResponse.ok) {
          ...
          console.warn("Failed to update order status, but continuing with display")
        }
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-2xl mx-auto">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-600">Processing your order and sending confirmation...</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const totalAmount = order.final_price || totalBookPrice + deliveryFee - discountAmount
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <Card className="border-green-200 bg-green-50">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-green-700 text-lg">Thank you for your purchase. Your order has been confirmed.</p>
              {emailSent && (
                <p className="text-green-600 text-sm mt-2">📧 Confirmation email sent to {order.customer_email}</p>
              )}
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-blue-800">{order.tracking_number}</div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>
                      Book{quantity > 1 ? "s" : ""}: "
                      {order.product_name || "Conquering Your Years in Tertiary Education"}" × {quantity}
                    </span>
                    <span>R{(totalBookPrice / 100).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Delivery ({order.delivery_method === "paxi" ? "PAXI" : "Door-to-Door"})</span>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                    <span className="text-blue-600 font-semibold">{order.customer_name.charAt(0).toUpperCase()}</span>
                  </div>
                  <div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                {order.delivery_method === "paxi" ? <Store className="h-5 w-5" /> : <Truck className="h-5 w-5" />}
                Delivery Information
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <div className="text-sm font-medium text-gray-600">Delivery Method</div>
                  <div className="capitalize">
                    {order.delivery_method === "paxi" ? "PAXI Collection" : "Door-to-Door Delivery"}
                  </div>
                </div>

                {order.delivery_method === "paxi" && order.paxi_code && (
                  <div>
                    <div className="text-sm font-medium text-gray-600">PEP Store Code</div>
                    <div className="font-mono text-lg">{order.paxi_code}</div>
                  </div>
                )}

                {order.delivery_method === "door_to_door" && order.delivery_address && (
                  <div>
              ...
              {order.delivery_method === "paxi" && (
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <div className="font-medium text-blue-800">PAXI Collection Instructions</div>
                      <div className="text-sm text-blue-700 mt-1">
                        You will receive an SMS from PEP with your collection details once your order is ready for
                        pickup at your selected PEP store.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      Your order is being prepared for shipment (1-2 business days)
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      {order.delivery_method === "paxi"
                        ? `Your book${quantity > 1 ? "s" : ""} will be sent to your selected PEP store`
                        : `Your book${quantity > 1 ? "s" : ""} will be shipped to your delivery address`}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <div className="font-medium">
                      {order.delivery_method === "paxi" ? "Collection Ready" : "Delivery"}
                    </div>
                    <div className="text-sm text-gray-600">
                      {order.delivery_method === "paxi"
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">majork.n07@gmail.com</div>
                  </div>
                </div>
